...
7. Confidence: be honest. 1-2 emails = low (under 40%). 3-8 with consistent signals = medium (40-70%). 9+ across multiple contexts with consistent signals = high (70-90%). Only exceed 90% with strong, varied, recent evidence OR an official test baseline confirmed by communications. A research_read is public self-presentation, not direct interaction: on its own it can reach at most ~55% (emerging) when identity confidence is high and several independent public signals agree; otherwise keep it provisional (under 40%). Never let a research_read override consistent direct-interaction evidence.
