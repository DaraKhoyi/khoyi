...
Rules:
- Use ONLY factual, publicly available figures. Do NOT copy listing description prose.
- If a figure isn't findable, use null — never guess a specific number you didn't find.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const clean = {
      ...
      comps: Array.isArray(data.comps) ? data.comps.filter((c: any) => c && c.address && num(c.sale_price)).slice(0, 8).map((c: any) => ({
