  ...
  try {
    switch (name) {
      ...
      case "research_contact": {
        ...
        const list = matches || [];
        if (list.length === 1) {
          if (ctx.actionRef) ctx.actionRef.value = { kind: "research", contact_id: list[0].id, name: list[0].name, hint: input.hint || null };
        ...
        const hasIdentifier = !!(input.email || input.phone || input.company || input.hint);
        ...
        if (ctx.actionRef) ctx.actionRef.value = { kind: "create", name: nm, email: input.email || null, phone: input.phone || null, company: input.company || null, hint: input.hint || null };
