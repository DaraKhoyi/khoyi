    ...
    if (recruiting_email) {
      try {
        ...
        if (acct && signed?.signedUrl) {
          await db.functions.invoke("gmail-send", { body: { account_id: acct.id, to: recruiting_email, subject: `CDA \u2014 ${doc_title || "Commission Disbursement"}${agent_name ? " \u2014 " + agent_name : ""}`, body_text: `A Commission Disbursement Authorization has been generated.\n\nAgent: ${agent_name || ""}\n\nView (link valid 7 days):\n${signed.signedUrl}\n\n\u2014 Realty ONE Group Advantage \u00B7 PrismOS` } });
