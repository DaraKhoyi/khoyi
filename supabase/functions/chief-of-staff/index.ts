...
async function generateForUser(sb: any, uid: string): Promise<number> {
  ...
  // WATCHER: owe-a-reply — the last thing that happened with this person was THEM reaching out
  try {
    const { data } = await sb.from("contacts").select("id,name,email,last_inbound_at,last_communication_channel").eq("user_id", uid).eq("last_communication_direction", "inbound").gte("last_inbound_at", daysAgo(21)).lte("last_inbound_at", new Date(now - 6 * 3600e3).toISOString()).order("last_inbound_at", { ascending: true }).limit(8);
    for (const c of (data || [])) { const days = c.last_inbound_at ? Math.floor((now - new Date(c.last_inbound_at).getTime()) / 864e5) : 0; const via = c.last_communication_channel ? ` (${c.last_communication_channel})` : ""; obligations.push({ idx: idx++, bucket: "obligation", kind: "owe_reply", title: `Reply to ${c.name}`, context: `they reached out${via} and haven't heard back ${days === 0 ? "yet today" : days + " day" + (days === 1 ? "" : "s") + " ago"} — the ball is in your court`, action_type: "reply_contact", source_ref: "reply:" + c.id, payload: { contact_id: c.id, email: c.email || null, name: c.name }, base: days >= 3 ? 1 : 2 }); }
  } catch (_) {}
  // WATCHER: pipeline stall — a live deal stuck at the same stage too long
  try {
    const { data } = await sb.from("deals").select("id,name,client_name,address,status,status_changed_at,updated_at").eq("user_id", uid).not("status", "in", "(closed,lost,dead,archived,withdrawn,cancelled,sold,lead)").limit(20);
    for (const d of (data || [])) { const anchor = d.status_changed_at || d.updated_at; if (!anchor) continue; const days = Math.floor((now - new Date(anchor).getTime()) / 864e5); if (days < 10) continue; obligations.push({ idx: idx++, bucket: "obligation", kind: "deal", title: `Deal stalling: ${d.name || d.client_name || d.address || "untitled"}`, context: `stuck at "${d.status || "active"}" for ${days} days with no movement — nudge it forward`, action_type: "review_deal", source_ref: "deal:" + d.id, payload: { deal_id: d.id }, base: days >= 21 ? 1 : 2 }); }
  } catch (_) {}
  try {
