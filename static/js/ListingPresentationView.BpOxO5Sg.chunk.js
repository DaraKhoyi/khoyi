import{r as z,j as e}from"./vendor-react.DecIrkRQ.chunk.js";import{s as K,d as ye,i as me}from"./main.B0xdUB2a.js";import"./vendor-react-dom.CLTjDK3B.chunk.js";import"./vendor-supabase.Bv9NtBh5.chunk.js";import"./vendor.Ks84_326.chunk.js";const he=[{n:1,label:"Full Rehab",say:"“Down to the studs.” Everything needs replacing — this sells as a project to investors or renovators."},{n:2,label:"Major Overhaul",say:"“Good bones, big to-do list.” Structurally sound, but systems and finishes are all at the end of their life."},{n:3,label:"Heavy Updating",say:"“A fixer with real upside.” Livable for now, but kitchen, baths, and mechanicals all need work."},{n:4,label:"Dated but Solid",say:"“Tired, not broken.” Everything functions — the look is a generation behind."},{n:5,label:"Cosmetic Refresh",say:"“Paint, floors, and polish.” A solid home that shows and photographs better with light updates."},{n:6,label:"Well-Kept & Classic",say:"“Loved and maintained — just not on-trend.” Move-in ready with a timeless rather than current style."},{n:7,label:"Move-In Ready",say:"“Bring your toothbrush.” Nothing needs doing; a buyer might modernize a few touches over time."},{n:8,label:"Turnkey Modern",say:"“Updated and easy to love.” Current finishes — nothing on the buyer’s list for years."},{n:9,label:"Designer-Done",say:"“Magazine-ready.” Recently and tastefully renovated; it shows like a model home."},{n:10,label:"Flawless",say:"“Not one thing to change.” A buyer moves in and wouldn’t touch a single detail."}],Z=i=>he.find(n=>n.n===Number(i))||he[6],ve={analytical:{eyebrow:"The evidence, in order",intro:"Every number here is defensible and sourced. Read it top to bottom — the pricing follows from the data, not the other way around."},direct:{eyebrow:"The bottom line first",intro:"Here is where your home stands, what it’s worth, and what you’ll net. The detail is below if you want it — but the headline is the number."},relational:{eyebrow:"Your home’s next chapter",intro:"You’ve made this house a home. Here’s how we honor that story while positioning it to sell for everything it’s worth — together."},auto:{eyebrow:"Your listing strategy",intro:"A complete, defensible plan for pricing, positioning, and netting the most from the sale of your home."}},u=i=>String(i??"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n]),A=i=>i||i===0?"$"+Number(i).toLocaleString("en-US",{maximumFractionDigits:0}):"—";function we(i){const n=i.subject||{},j=i.market||{},D=i.comps||[],R=i.tiers||{},m=i.netsheet||{},_=Z(n.condition_score),s=ve[i.seller_tone]||ve.auto,b=u(i.address||"Your Property"),w=u(i.agent_name||""),S="Realty ONE Group Advantage",P=u(i.seller_name||""),$=(i.hero_image_url||"").trim(),l=(i.agent_video_url||"").trim(),C=!!i.sign_mode,U=u(i.share_token||""),E=(()=>{if(!l)return"";const a=l.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/))([\w-]+)/);if(a)return`<iframe src="https://www.youtube.com/embed/${a[1]}" allow="fullscreen" style="width:100%;aspect-ratio:16/9;border:0;border-radius:14px"></iframe>`;const v=l.match(/vimeo\.com\/(\d+)/);return v?`<iframe src="https://player.vimeo.com/video/${v[1]}" allow="fullscreen" style="width:100%;aspect-ratio:16/9;border:0;border-radius:14px"></iframe>`:/\.(mp4|webm|mov)(\?|$)/i.test(l)?`<video src="${u(l)}" controls playsinline style="width:100%;border-radius:14px;background:#000"></video>`:""})(),W=Number(m.commission_pct??6)/100,d=a=>{if(!a)return null;const v=a*W,x=Number(m.title_fees||0),t=Number(m.tax_proration||0),r=Number(m.mortgage_payoff||0),h=Number(m.other||0);return a-v-x-t-r-h},V=D.filter(a=>Number(a.sale_price)&&Number(a.gla)).map(a=>Number(a.sale_price)/Number(a.gla)),J=a=>{if(!a.length)return null;const v=[...a].sort((t,r)=>t-r),x=Math.floor(v.length/2);return v.length%2?v[x]:(v[x-1]+v[x])/2},ee=a=>Math.round(a/1e3)*1e3;let M=null;const O=J(V);if(O&&Number(n.gla))M=O*Number(n.gla);else{const a=J(D.map(v=>Number(v.sale_price)).filter(Boolean));a&&(M=a)}const H=(a,v)=>{const x=Number(a||0);return x>0?x:M&&M>0?ee(M*v):0},q=[{key:"opportunistic",label:"Opportunistic",sub:"Test the ceiling",price:H(R.opportunistic,1.06),dom:"Longer",prob:"Lower"},{key:"target",label:"Target Market",sub:"Priced to sell right",price:H(R.target,1),dom:"Market pace",prob:"Strong"},{key:"fast",label:"Fast Sale",sub:"Move it quickly",price:H(R.fast,.95),dom:"Fastest",prob:"Highest"}],X=D.map((a,v)=>{const x=(a.adjustments||[]).filter(o=>Number(o.amount)!==0),t=x.reduce((o,f)=>o+Number(f.amount||0),0),r=Number(a.sale_price||0)+t,h=x.length?`<div class="adj-items">${x.map(o=>`<span class="adj-item"><span class="adj-lab">${u(o.label)}</span><span class="${Number(o.amount)>=0?"pos":"neg"}">${Number(o.amount)>=0?"+":"−"}${A(Math.abs(Number(o.amount)))}</span></span>`).join("")}</div>`:"";return`<tr>
      <td class="comp-addr">${u(a.address||"Comp "+(v+1))}${h}</td>
      <td>${A(a.sale_price)}</td>
      <td>${a.gla?u(a.gla)+" sf":"—"}</td>
      <td class="${t>=0?"pos":"neg"}">${t>=0?"+":"−"}${A(Math.abs(t))}</td>
      <td class="adjusted">${A(r)}</td>
    </tr>`}).join(""),L=[];return n.gla&&L.push(["GLA",u(n.gla)+" sf"]),n.beds&&L.push(["Beds",u(n.beds)]),n.baths&&L.push(["Baths",u(n.baths)]),n.lot_size&&L.push(["Lot",u(n.lot_size)]),n.year_built&&L.push(["Built",u(n.year_built)]),L.push(["Condition",_.n+"/10 · "+u(_.label)]),`<!doctype html><html lang="en"><head>
...
<body>
  <header class="hero${$?" has-photo":""}"${$?` style="background-image:linear-gradient(180deg,rgba(16,13,9,.55),rgba(16,13,9,.92)),url('${u($)}')"`:""}><div class="wrap">
    <div class="eyebrow">${P?"Prepared exclusively for "+P:u(s.eyebrow)}</div>
    <h1>${b}</h1>
    <div class="sub">Executive Listing Presentation</div>
    <div class="badges">${L.map(([a,v])=>`<span class="badge"><b>${a}</b>${v}</span>`).join("")}</div>
    <div class="brandline">
  ...
  <section><div class="wrap">
    ...
    <div class="card" style="margin-top:20px">
      ...
      <div class="cond" style="margin-top:12px">
        ...
        <div><div style="font-family:Fraunces,serif;font-size:26px;color:var(--champ)">${u(_.label)}</div>
        <div style="color:#c9bfa9;margin-top:4px;max-width:52ch">${u(_.say)}</div></div>
      </div>
    </div>
  </div></section>
  ...
  <section><div class="wrap">
    ...
    <div class="grid3" style="margin-top:26px">
      ...
      <div class="stat"><div class="k">List-to-Sale</div><div class="v">${j.list_to_sale?u(j.list_to_sale)+"%":"—"}</div></div>
      <div class="stat"><div class="k">Active / Pending / Closed</div><div class="v" style="font-size:22px">${u(j.active||"—")} / ${u(j.pending||"—")} / ${u(j.closed||"—")}</div></div>
    </div>
  </div></section>
  ...
  <section><div class="wrap">
    ...
    ${D.length?`<table><thead><tr><th>Comparable</th><th>Sold</th><th>Size</th><th>Adjustment</th><th>Adjusted</th></tr></thead><tbody>${X}</tbody></table>`:'<p style="color:var(--mut);margin-top:20px">Comparable properties will be added here.</p>'}
  </div></section>
  ...
  <section><div class="wrap">
    ...
    <div class="tiers" style="margin-top:24px">
      ${q.map(a=>`<div class="tier ${a.key==="target"?"target":""}">${a.key==="target"?'<div class="flag">Recommended</div>':""}
        <div class="tlab">${a.label}</div><div class="price">${a.price>0?A(a.price):'<span style="font-size:20px;color:var(--mut);font-weight:400">To be set</span>'}</div><div class="sub">${a.sub}</div>
  ...
  <section><div class="wrap">
    ...
    <div class="equity">
      ...
      ${q.filter(a=>a.price).map(a=>{const v=d(a.price),x=a.price-(v||0);return`<div class="er">${a.label}</div><div class="ev">${A(a.price)}</div><div class="ev">−${A(x)}</div><div class="en">${A(v)}</div>`}).join("")}
    </div>
    <div class="card" style="margin-top:20px">
...
<script>
  var NS = ${JSON.stringify({commissionPct:W,title:Number(m.title_fees||0),tax:Number(m.tax_proration||0),payoff:Number(m.mortgage_payoff||0),other:Number(m.other||0)})};
  ...
  (function(){
    ...
    window.doSign=async function(){
      ...
      try{
        var res=await fetch('${i.supabase_url||""}/functions/v1/listing-present?sign=1&t=${U}',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:name,drawn:c.toDataURL('image/png'),consent:consent,ua:navigator.userAgent})});
...
<\/script>
</body></html>`}const B="#CBA35C",Q="#EBCB82",xe="#100D09",je=i=>i||i===0?"$"+Number(i).toLocaleString("en-US"):"",ke=()=>(crypto.randomUUID?crypto.randomUUID():Math.random().toString(36).slice(2)).replace(/-/g,"").slice(0,22),fe=i=>{if(!i)return"";const n=Math.floor((Date.now()-new Date(i).getTime())/1e3);return n<60?"just now":n<3600?Math.floor(n/60)+"m ago":n<86400?Math.floor(n/3600)+"h ago":Math.floor(n/86400)+"d ago"};function Te({userId:i,agentName:n}){const[j,D]=z.useState(null),[R,m]=z.useState("list"),[_,s]=z.useState(null),[b,w]=z.useState(null),S=z.useCallback(async()=>{const{data:l}=await K.from("listing_presentations").select("*").eq("user_id",i).order("updated_at",{ascending:!1});D(l||[])},[i]);z.useEffect(()=>{S()},[S]);const P=()=>({title:"",address:"",contact_id:null,deal_id:null,seller_tone:"auto",seller_name:"",hero_image_url:"",agent_video_url:"",subject:{gla:"",beds:"",baths:"",lot_size:"",year_built:"",condition_score:7,upgrades:"",hidden_changes:"",motivation:""},market:{speed:55,moi:"",list_to_sale:"",active:"",pending:"",closed:""},comps:[{address:"",sale_price:"",gla:"",adjustments:[]}],tiers:{opportunistic:"",target:"",fast:""},netsheet:{commission_pct:6,mortgage_payoff:"",title_fees:"",tax_proration:"",other:""}}),$=(l,C=!0)=>{w({t:l,ok:C}),setTimeout(()=>w(null),4e3)};return R==="edit"?e.jsx(_e,{initial:_,userId:i,agentName:n,onDone:()=>{m("list"),S()},onCancel:()=>m("list"),flash:$,notify:b}):e.jsxs("div",{style:{maxWidth:900,margin:"0 auto",padding:"0 4px 40px"},children:[e.jsx(ye,{screen:"listing_presentation"}),b&&e.jsx("div",{style:{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",zIndex:5e3,background:b.ok?"#1a3a2a":"#3a1a1a",border:`1px solid ${b.ok?"#7fae8f":"#e0794f"}`,color:"#fff",padding:"12px 18px",borderRadius:10,fontSize:14,maxWidth:"90vw"},children:b.t}),e.jsx("div",{style:{marginBottom:6},children:e.jsx("span",{style:{fontFamily:"Barlow Condensed,sans-serif",fontWeight:700,letterSpacing:".2em",textTransform:"uppercase",color:B,fontSize:13},children:"Win the listing"})}),e.jsx("h1",{style:{fontFamily:"Fraunces,serif",fontWeight:400,fontSize:34,color:"var(--text-1)",margin:"0 0 6px"},children:"Listing Presentations"}),e.jsx("p",{style:{color:"var(--text-2)",fontSize:15,margin:"0 0 20px",maxWidth:"62ch"},children:"Turn an address into an executive, DISC-aware valuation dossier — pricing, comps, a launch plan, and a live net-sheet — as a branded web presentation you can present, email, or share with the seller."}),e.jsx("button",{onClick:()=>{s(P()),m("edit")},style:{background:Q,color:xe,border:"none",borderRadius:10,padding:"13px 22px",fontWeight:800,fontSize:15,cursor:"pointer"},children:"+ New Listing Presentation"}),e.jsx("div",{style:{marginTop:28},children:j===null?e.jsx("div",{style:{color:"var(--text-3)"},children:"Loading…"}):j.length===0?e.jsx("div",{style:{color:"var(--text-3)",fontSize:14,border:"1px dashed var(--border)",borderRadius:12,padding:"26px",textAlign:"center"},children:"No presentations yet. Build your first one — it takes about five minutes."}):j.map(l=>{var C,U,E,W;return e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,padding:"14px 16px",marginBottom:10},children:[e.jsxs("div",{style:{minWidth:0},children:[e.jsx("div",{style:{fontWeight:700,color:"var(--text-1)",fontSize:15,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"},children:l.title||l.address||"Untitled presentation"}),e.jsxs("div",{style:{fontSize:12,color:"var(--text-3)",marginTop:2},children:[Z((C=l.subject)==null?void 0:C.condition_score).n,"/10 · ",Z((U=l.subject)==null?void 0:U.condition_score).label,(E=l.tiers)!=null&&E.target?` · target ${je(l.tiers.target)}`:"",l.share_enabled?" · 🔗 shared":"",l.view_count?` · ${l.view_count} views`:""]}),l.last_viewed_at&&e.jsxs("div",{style:{fontSize:12,color:"#7fae8f",marginTop:3,fontWeight:600},children:["👁 Seller viewed ",fe(l.last_viewed_at)]}),l.signed_at&&e.jsxs("div",{style:{fontSize:12,color:Q,marginTop:3,fontWeight:700},children:["✍️ Signed ",fe(l.signed_at),(W=l.signature)!=null&&W.name?` by ${l.signature.name}`:""]})]}),e.jsx("div",{style:{display:"flex",gap:6,flexShrink:0},children:e.jsx("button",{onClick:()=>{s(l),m("edit")},style:Y,children:"Open"})})]},l.id)})})]})}const Y={background:"var(--bg-base)",border:"1px solid var(--border)",color:"var(--text-2)",borderRadius:8,padding:"8px 14px",fontSize:13,fontWeight:700,cursor:"pointer"},c={width:"100%",boxSizing:"border-box",background:"var(--bg-base)",border:"1px solid var(--border)",borderRadius:9,color:"var(--text-1)",padding:"10px 12px",fontSize:14},g={display:"block",fontSize:11,fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",color:"var(--text-3)",margin:"0 0 6px"},I={fontFamily:"Barlow Condensed,sans-serif",fontWeight:700,letterSpacing:".16em",textTransform:"uppercase",color:B,fontSize:12,margin:"22px 0 10px"};function _e({initial:i,userId:n,agentName:j,onDone:D,onCancel:R,flash:m,notify:_}){var v,x;const[s,b]=z.useState(i),[w,S]=z.useState(!1),P=async()=>{var r,h,o;const t=(s.address||"").trim();if(!t){m("Enter the property address first.",!1);return}S(!0);try{const f=[((r=s.subject)==null?void 0:r.beds)&&`${s.subject.beds} bed`,((h=s.subject)==null?void 0:h.baths)&&`${s.subject.baths} bath`,((o=s.subject)==null?void 0:o.gla)&&`${s.subject.gla} sqft`].filter(Boolean).join(", "),{data:N,error:G}=await K.functions.invoke("property-research",{body:{user_id:n,address:t,subject_hint:f}});if(G||!(N!=null&&N.ok)){m("Research came up empty — try again, or enter the numbers by hand below.",!1),S(!1);return}const y=N.data;b(k=>{var ae,re,ie,ne,oe,le,de,ce,pe,ue,ge;const T={...k};T.subject={...k.subject};for(const p of["gla","beds","baths","lot_size","year_built"])(((ae=k.subject)==null?void 0:ae[p])===""||((re=k.subject)==null?void 0:re[p])==null)&&((ie=y.subject)==null?void 0:ie[p])!=null&&(T.subject[p]=y.subject[p]);T.market={...k.market},((ne=y.market)==null?void 0:ne.speed)!=null&&(T.market.speed=y.market.speed);for(const[p,F]of[["moi","moi"],["list_to_sale","list_to_sale"],["active","active"],["pending","pending"],["closed","closed"]])(((oe=k.market)==null?void 0:oe[p])===""||((le=k.market)==null?void 0:le[p])==null)&&((de=y.market)==null?void 0:de[F])!=null&&(T.market[p]=y.market[F]);const te=(y.comps||[]).map(p=>({address:p.address||"",sale_price:p.sale_price||"",gla:p.gla||"",beds:p.beds??"",baths:p.baths??"",year_built:p.year_built??"",adjustments:Array.isArray(p.adjustments)?p.adjustments.map(F=>({label:F.label,amount:F.amount})):[]}));if(te.length){const p=(k.comps||[]).some(F=>(F.address||"").trim()||Number(F.sale_price)>0);T.comps=p?[...k.comps,...te]:te}const se=((ce=y.valuation)==null?void 0:ce.tiers)||{};T.tiers={...k.tiers};for(const p of["opportunistic","target","fast"])(((pe=k.tiers)==null?void 0:pe[p])===""||((ue=k.tiers)==null?void 0:ue[p])==null||Number((ge=k.tiers)==null?void 0:ge[p])===0)&&se[p]!=null&&(T.tiers[p]=se[p]);return T._research={sources:y.sources||[],notes:y.notes||"",confidence:y.confidence||"low",valuation:y.valuation||null},T});const be=y.confidence==="high"?"strong":y.confidence==="medium"?"decent":"limited";m(`Pulled ${(y.comps||[]).length} comp(s) + market data from public sources (${be} match). Review and adjust the numbers below before presenting.`,!0)}catch{m("Research failed — please try again.",!1)}S(!1)},[$,l]=z.useState(!1),[C,U]=z.useState(i.id||null),[E,W]=z.useState(null),d=(t,r)=>b(h=>{const o={...h},f=t.split(".");return f.length===1?o[f[0]]=r:o[f[0]]={...o[f[0]],[f[1]]:r},o}),V=(t,r,h)=>b(o=>{const f=[...o.comps];return f[t]={...f[t],[r]:h},{...o,comps:f}}),J=()=>b(t=>({...t,comps:[...t.comps,{address:"",sale_price:"",gla:"",adjustments:[]}]})),ee=t=>b(r=>({...r,comps:r.comps.filter((h,o)=>o!==t)})),M=Z(s.subject.condition_score),O=t=>{const r={};for(const h in t){const o=t[h];r[h]=o===""||o==null?null:isNaN(Number(o))?o:Number(o)}return r},H=()=>{var t,r,h,o,f,N;return{user_id:n,contact_id:s.contact_id||null,deal_id:s.deal_id||null,title:((t=s.title)==null?void 0:t.trim())||((r=s.address)!=null&&r.trim()?s.address.trim()+" — Listing Presentation":"Untitled presentation"),address:((h=s.address)==null?void 0:h.trim())||null,seller_tone:s.seller_tone,seller_name:((o=s.seller_name)==null?void 0:o.trim())||null,hero_image_url:((f=s.hero_image_url)==null?void 0:f.trim())||null,agent_video_url:((N=s.agent_video_url)==null?void 0:N.trim())||null,subject:O(s.subject),market:O(s.market),comps:(s.comps||[]).map(G=>({...G,sale_price:Number(G.sale_price)||0,gla:G.gla||null,adjustments:G.adjustments||[]})),tiers:O(s.tiers),netsheet:O(s.netsheet)}},q=(t={})=>we({...H(),agent_name:j,supabase_url:me,...t}),X=async()=>{l(!0);const t={...H(),html:q(),updated_at:new Date().toISOString()};let r;return C?r=await K.from("listing_presentations").update(t).eq("id",C).select().single():r=await K.from("listing_presentations").insert(t).select().single(),r.error?(m("Could not save: "+r.error.message,!1),l(!1),null):(U(r.data.id),l(!1),m("Saved."),r.data)},L=async()=>{const t=await X();if(!t)return;const r=window.open("","_blank");r?(r.document.write(t.html||q()),r.document.close()):m("Allow pop-ups to preview, or use Share/Email.",!1)},a=async()=>{const t=await X();if(!t)return;const r=t.share_token||ke(),h=q({sign_mode:!0,share_token:r}),{data:o,error:f}=await K.from("listing_presentations").update({share_token:r,share_enabled:!0,html:h}).eq("id",t.id).select().single();if(f){m("Could not enable sharing: "+f.message,!1);return}const N=`${me}/functions/v1/listing-present?t=${o.share_token}`;W(N);try{await navigator.clipboard.writeText(N),m("Share link copied — send it to your seller.")}catch{m("Share link ready (copy it below).")}};return e.jsxs("div",{style:{maxWidth:780,margin:"0 auto",padding:"0 4px 60px"},children:[_&&e.jsx("div",{style:{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",zIndex:5e3,background:_.ok?"#1a3a2a":"#3a1a1a",border:`1px solid ${_.ok?"#7fae8f":"#e0794f"}`,color:"#fff",padding:"12px 18px",borderRadius:10,fontSize:14,maxWidth:"90vw"},children:_.t}),e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:16},children:[e.jsxs("div",{children:[e.jsxs("div",{style:{fontFamily:"Barlow Condensed,sans-serif",fontWeight:700,letterSpacing:".2em",textTransform:"uppercase",color:B,fontSize:12},children:[C?"Editing":"New"," presentation"]}),e.jsx("h1",{style:{fontFamily:"Fraunces,serif",fontWeight:400,fontSize:26,color:"var(--text-1)",margin:"2px 0 0"},children:s.address||"Listing Presentation"})]}),e.jsx("button",{onClick:R,style:Y,children:"← Back"})]}),e.jsx("div",{style:I,children:"The property"}),e.jsxs("div",{style:{marginBottom:12},children:[e.jsx("label",{style:g,children:"Property address"}),e.jsxs("div",{style:{display:"flex",gap:8},children:[e.jsx("input",{style:{...c,flex:1},value:s.address,onChange:t=>d("address",t.target.value),placeholder:"4214 W Virginia Ave, Tampa, FL 33607"}),e.jsx("button",{onClick:P,disabled:w,title:"Research comps, market data & property facts from public sources (Zillow, Realtor, Redfin, county records)",style:{...Y,whiteSpace:"nowrap",opacity:w?.6:1,cursor:w?"wait":"pointer"},children:w?"⏳ Researching…":"✨ Auto-research"})]}),e.jsx("div",{style:{fontSize:11,color:"var(--text-3)",marginTop:5},children:"Pulls comparable sales, market speed and property facts from public sources (Zillow, Realtor.com, Redfin, county records) — a stand-in until your IDX feed is connected. Always review the numbers before presenting."}),s._research&&((v=s._research.sources)!=null&&v.length||s._research.notes)?e.jsxs("div",{style:{fontSize:11,color:"var(--text-3)",marginTop:6,padding:"8px 10px",background:"rgba(203,163,92,.06)",border:"1px solid rgba(203,163,92,.18)",borderRadius:8},children:[e.jsxs("b",{style:{color:"var(--text-2)"},children:["Research match: ",s._research.confidence,"."]})," ",s._research.notes?s._research.notes.slice(0,220):"",(x=s._research.sources)!=null&&x.length?e.jsxs("div",{style:{marginTop:4},children:["Sources: ",s._research.sources.slice(0,4).map((t,r)=>e.jsxs("a",{href:t,target:"_blank",rel:"noreferrer",style:{color:"var(--accent)",marginRight:8},children:["[",r+1,"]"]},r))]}):null]}):null]}),e.jsxs("div",{style:{marginBottom:12},children:[e.jsx("label",{style:g,children:"Seller name (personalizes the cover)"}),e.jsx("input",{style:c,value:s.seller_name,onChange:t=>d("seller_name",t.target.value),placeholder:"The Henderson Family"})]}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12},children:[e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Hero photo URL"}),e.jsx("input",{style:c,value:s.hero_image_url,onChange:t=>d("hero_image_url",t.target.value),placeholder:"https://…/front.jpg"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Agent welcome video"}),e.jsx("input",{style:c,value:s.agent_video_url,onChange:t=>d("agent_video_url",t.target.value),placeholder:"YouTube / Vimeo / .mp4 link"})]})]}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:12},children:[e.jsxs("div",{children:[e.jsx("label",{style:g,children:"GLA (sq ft)"}),e.jsx("input",{style:c,value:s.subject.gla,onChange:t=>d("subject.gla",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Beds"}),e.jsx("input",{style:c,value:s.subject.beds,onChange:t=>d("subject.beds",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Baths"}),e.jsx("input",{style:c,value:s.subject.baths,onChange:t=>d("subject.baths",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Lot"}),e.jsx("input",{style:c,value:s.subject.lot_size,onChange:t=>d("subject.lot_size",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Year built"}),e.jsx("input",{style:c,value:s.subject.year_built,onChange:t=>d("subject.year_built",t.target.value)})]})]}),e.jsx("div",{style:I,children:"Property condition — 1 to 10"}),e.jsxs("div",{style:{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,padding:"18px"},children:[e.jsxs("div",{style:{display:"flex",alignItems:"baseline",gap:12,marginBottom:12},children:[e.jsxs("div",{style:{fontFamily:"Fraunces,serif",fontSize:42,color:B,lineHeight:1},children:[M.n,e.jsx("span",{style:{fontSize:20,color:"var(--text-3)"},children:"/10"})]}),e.jsx("div",{style:{fontFamily:"Fraunces,serif",fontSize:22,color:Q},children:M.label})]}),e.jsx("input",{type:"range",min:"1",max:"10",step:"1",value:s.subject.condition_score,onChange:t=>d("subject.condition_score",Number(t.target.value)),style:{width:"100%",accentColor:B}}),e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",fontSize:10,color:"var(--text-3)",marginTop:2},children:[e.jsx("span",{children:"1 · Full rehab"}),e.jsx("span",{children:"10 · Flawless"})]}),e.jsx("div",{style:{marginTop:12,fontSize:14,color:"var(--text-2)",fontStyle:"italic",lineHeight:1.5,borderLeft:`2px solid ${B}`,paddingLeft:12},children:M.say})]}),e.jsxs("div",{style:{marginTop:12},children:[e.jsx("label",{style:g,children:"Upgrades & improvements"}),e.jsx("textarea",{style:{...c,minHeight:70,resize:"vertical"},value:s.subject.upgrades,onChange:t=>d("subject.upgrades",t.target.value),placeholder:"New roof (2024), quartz kitchen, impact windows, renovated primary bath…"})]}),e.jsxs("div",{style:{marginTop:12},children:[e.jsx("label",{style:g,children:"Changes not in public record"}),e.jsx("textarea",{style:{...c,minHeight:70,resize:"vertical"},value:s.subject.hidden_changes,onChange:t=>d("subject.hidden_changes",t.target.value),placeholder:"Lanai permitted & converted to a 4th bedroom; garage insulated as flex office; well added for irrigation…"})]}),e.jsxs("div",{style:{marginTop:12},children:[e.jsx("label",{style:g,children:"Owner motivation / objective"}),e.jsx("input",{style:c,value:s.subject.motivation,onChange:t=>d("subject.motivation",t.target.value),placeholder:"Relocating for work — needs to close by spring"})]}),e.jsx("div",{style:I,children:"Seller’s style (DISC framing)"}),e.jsx("div",{style:{display:"flex",gap:8,flexWrap:"wrap"},children:[["auto","Auto"],["analytical","Analytical (C)"],["direct","Direct (D)"],["relational","Relational (I/S)"]].map(([t,r])=>e.jsx("button",{onClick:()=>d("seller_tone",t),style:{padding:"7px 14px",borderRadius:100,cursor:"pointer",fontSize:13,fontWeight:s.seller_tone===t?700:500,border:`1px solid ${s.seller_tone===t?B:"var(--border)"}`,background:s.seller_tone===t?"rgba(203,163,92,.15)":"transparent",color:s.seller_tone===t?B:"var(--text-2)"},children:r},t))}),e.jsx("div",{style:I,children:"Micro-market"}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10},children:[e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Months of inventory"}),e.jsx("input",{style:c,value:s.market.moi,onChange:t=>d("market.moi",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"List-to-sale %"}),e.jsx("input",{style:c,value:s.market.list_to_sale,onChange:t=>d("market.list_to_sale",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Active"}),e.jsx("input",{style:c,value:s.market.active,onChange:t=>d("market.active",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Pending"}),e.jsx("input",{style:c,value:s.market.pending,onChange:t=>d("market.pending",t.target.value)})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Closed (90d)"}),e.jsx("input",{style:c,value:s.market.closed,onChange:t=>d("market.closed",t.target.value)})]})]}),e.jsxs("div",{style:{marginTop:10},children:[e.jsxs("label",{style:g,children:["Market speed (buyer’s ← → seller’s): ",s.market.speed]}),e.jsx("input",{type:"range",min:"0",max:"100",value:s.market.speed,onChange:t=>d("market.speed",Number(t.target.value)),style:{width:"100%",accentColor:B}})]}),e.jsx("div",{style:I,children:"Comparables"}),s.comps.map((t,r)=>e.jsxs("div",{style:{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:10,padding:12,marginBottom:8},children:[e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:8},children:[e.jsxs("span",{style:{fontSize:12,color:"var(--text-3)",fontWeight:700},children:["Comp ",r+1]}),s.comps.length>1&&e.jsx("button",{onClick:()=>ee(r),style:{background:"none",border:"none",color:"#e0794f",cursor:"pointer",fontSize:12},children:"Remove"})]}),e.jsx("input",{style:{...c,marginBottom:8},value:t.address,onChange:h=>V(r,"address",h.target.value),placeholder:"Comp address"}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8},children:[e.jsx("input",{style:c,value:t.sale_price,onChange:h=>V(r,"sale_price",h.target.value),placeholder:"Sale price",inputMode:"numeric"}),e.jsx("input",{style:c,value:t.gla,onChange:h=>V(r,"gla",h.target.value),placeholder:"GLA (sq ft)",inputMode:"numeric"})]})]},r)),e.jsx("button",{onClick:J,style:{...Y,marginBottom:4},children:"+ Add comp"}),e.jsx("div",{style:I,children:"Three-tier pricing"}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8},children:[e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Opportunistic"}),e.jsx("input",{style:c,value:s.tiers.opportunistic,onChange:t=>d("tiers.opportunistic",t.target.value),inputMode:"numeric",placeholder:"729000"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Target ★"}),e.jsx("input",{style:c,value:s.tiers.target,onChange:t=>d("tiers.target",t.target.value),inputMode:"numeric",placeholder:"699000"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Fast sale"}),e.jsx("input",{style:c,value:s.tiers.fast,onChange:t=>d("tiers.fast",t.target.value),inputMode:"numeric",placeholder:"669000"})]})]}),e.jsx("div",{style:I,children:"Seller net sheet"}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10},children:[e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Commission %"}),e.jsx("input",{style:c,value:s.netsheet.commission_pct,onChange:t=>d("netsheet.commission_pct",t.target.value),inputMode:"numeric"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Mortgage payoff"}),e.jsx("input",{style:c,value:s.netsheet.mortgage_payoff,onChange:t=>d("netsheet.mortgage_payoff",t.target.value),inputMode:"numeric"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Title & closing"}),e.jsx("input",{style:c,value:s.netsheet.title_fees,onChange:t=>d("netsheet.title_fees",t.target.value),inputMode:"numeric"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Tax proration"}),e.jsx("input",{style:c,value:s.netsheet.tax_proration,onChange:t=>d("netsheet.tax_proration",t.target.value),inputMode:"numeric"})]}),e.jsxs("div",{children:[e.jsx("label",{style:g,children:"Other"}),e.jsx("input",{style:c,value:s.netsheet.other,onChange:t=>d("netsheet.other",t.target.value),inputMode:"numeric"})]})]}),e.jsxs("div",{style:{position:"sticky",bottom:0,background:"linear-gradient(180deg,transparent,var(--bg-base) 30%)",paddingTop:20,marginTop:24},children:[e.jsxs("div",{style:{display:"flex",gap:8,flexWrap:"wrap"},children:[e.jsx("button",{onClick:X,disabled:$,style:{...Y,flex:"1 1 100px"},children:$?"Saving…":"Save"}),e.jsx("button",{onClick:L,disabled:$,style:{background:Q,color:xe,border:"none",borderRadius:9,padding:"11px 20px",fontWeight:800,fontSize:14,cursor:"pointer",flex:"2 1 160px"},children:"▶ Preview presentation"}),e.jsx("button",{onClick:a,disabled:$,style:{...Y,flex:"1 1 120px"},children:"🔗 Share with seller"})]}),E&&e.jsx("div",{style:{marginTop:10,background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:8,padding:"10px 12px",fontSize:12,color:"var(--text-2)",wordBreak:"break-all"},children:E})]})]})}export{Te as default};
