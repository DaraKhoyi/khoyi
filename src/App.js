...
const BrokerGoalRoster = lazyWithReload(() => import('./views/BrokerGoalRoster'));  const NightReview = lazyWithReload(() => import('./views/NightReview'));  const LaunchersView = lazyWithReload(() => import('./views/LaunchersView'));
...
function AppMain() {
  ...
  // reason.
  const builtSet = new Set([...NAV.map(i => i.id), 'disc_test', 'disc_roster', 'myvoice', 'voice_roster', 'google_contacts', 'cadence_review', 'goal_roster', 'night_review', 'uncarded', 'lead_notify', 'unstuck', 'tags', 'correspondent', 'investor_transition', 'my_prism', 'production', 'transactions', 'briefing', 'scoreboard', 'launchers']);
