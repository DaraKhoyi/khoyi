      ...
      <div className="modal" style={{maxWidth:'640px'}}>
        ...
        <div style={{maxHeight:'70vh',overflowY:'auto',paddingRight:'4px'}}>
          ...
          <div style={{marginTop:'18px',paddingTop:'14px',borderTop:'1px solid var(--border)'}}>
            ...
            <CustomFieldsPanel userId={userId} contact={contact} contacts={contacts} />
          </div>
        </div>
...
function CustomFieldsPanel({ userId, contact, contacts = [] }) {
...
function CustomFieldRow({ def, value, contacts, systems, saving, flash, onSave, onDelete }) {
  ...
  switch (def.field_type) {
    ...
    case 'contact_ref_multi': {
      // V1: simple text fallback. Splits comma-separated names. v2 will be a proper picker.
      const arr = Array.isArray(local) ? local : [];
      const display = arr.map(id => {
        const c = (contacts || []).find(x => x.id === id);
        return c ? c.name : id.slice(0, 6);
      }).join(', ');
      editor = (
        <input type="text" value={display} readOnly
          style={{...inputBase, fontStyle:'italic', color:'var(--text-3)'}}
          placeholder={def.placeholder || 'Multi-contact picker coming soon'}/>
      ); break;
    }
    default:
      editor = (
