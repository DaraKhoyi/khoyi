...
const BrokerGoalRoster = lazyWithReload(() => import('./views/BrokerGoalRoster'));  // goal vs pace
...
function AppMain() {
  ...
  // reason.
  const builtSet = new Set([...NAV.map(i => i.id), 'disc_test', 'disc_roster', 'myvoice', 'voice_roster', 'google_contacts', 'cadence_review', 'goal_roster', 'uncarded', 'lead_notify', 'unstuck', 'tags', 'correspondent', 'investor_transition', 'my_prism', 'production', 'transactions']);
        ...
        <main className="main-content ww-prism" ref={mainScrollRef} onTouchStart={onMainTouchStart} onTouchMove={onMainTouchMove} onTouchEnd={onMainTouchEnd} style={activeMode ? { paddingBottom: 'calc(var(--modebar-h, 76px) + 8px)' } : undefined}>
          ...
          {!dataLoaded
            ...
            : <ViewErrorBoundary key={view} viewName={view}>
              ...
              : view==='goal_roster' ? <BrokerGoalRoster/> : view==='uncarded' ? <UncardedView setView={setView}/>
